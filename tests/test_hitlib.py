...
def test_get_lines_from_file():
    ...
    output2 = {"a": 7, "": 3, "ap": 10}
    assert output == get_lines_from_file("tests/superman.txt")
    assert output2 == get_lines_from_file("tests/ap.txt")


def test_find_max_and_min_key():
    output = ("", 3, "ap", 10)
    assert output == find_max_and_min_key(get_lines_from_file("tests/ap.txt"))
